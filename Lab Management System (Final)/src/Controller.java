...
public class Controller {
    ...
    // Get all complaints for ViewComplaintStatusPage
    public List<ViewComplaintStatusPage.Complaint> getAllComplaints() {
        // Always try database first
        if (!DatabaseManager.isConnected()) {
            DatabaseManager.initialize();
        }
        ...
        try {
            List<ViewComplaintStatusPage.Complaint> complaints = DatabaseManager.getAllComplaints();
            if (complaints != null && !complaints.isEmpty()) {
                return complaints;
            }
        } catch (Exception e) {
            System.err.println("Error retrieving all complaints from database: " + e.getMessage());
        }
        
        // Return empty list if database retrieval fails
        return new ArrayList<>();
    }

    // Initialize sample complaints data in the database
    public void initializeSampleComplaints() {
        if (!DatabaseManager.isConnected()) {
            DatabaseManager.initialize();
        }
        
        // Add sample complaints
        DatabaseManager.addFormattedComplaint("C001", "Monitor not working", "Pending", "Student", "Display turns black after 10 minutes.");
        DatabaseManager.addFormattedComplaint("C002", "Software crash", "In Progress", "Teacher", "NetBeans crashes when saving.");
        DatabaseManager.addFormattedComplaint("C003", "Mouse issue", "Resolved", "Student", "Mouse is unresponsive.");
        DatabaseManager.addFormattedComplaint("C004", "Printer offline", "Pending", "Teacher", "Cannot connect to printer.");
        DatabaseManager.addFormattedComplaint("C005", "Slow PC", "Resolved", "Student", "System slows down while multitasking.");
    }
    ...
    public int getNextComplaintId() {
        ...
        try {
            return DatabaseManager.getNextComplaintId();
        } catch (Exception e) {
            System.err.println("Error retrieving next complaint ID: " + e.getMessage());
            // Fallback to a random ID if database access fails
            return (int)(Math.random() * 900 + 100);
        }
    }
    
    // Register a formatted complaint directly
    public boolean registerFormattedComplaint(String id, String description, String status, 
        ...
        try {
            return DatabaseManager.addFormattedComplaint(id, description, status, registeredBy, registeredDetails);
        } catch (Exception e) {
            System.err.println("Error registering formatted complaint: " + e.getMessage());
            return false;
        }
    }
}
