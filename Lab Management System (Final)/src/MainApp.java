...
public class MainApp extends Application {
    ...
    @Override
    public void stop() {
        // Properly close the database connection when the application exits
    ...
    public void showApproveRejectRequestsScreen(User user) {
        try {
            ApproveRejectRequestsPage page = new ApproveRejectRequestsPage();
            page.setMainApp(this);
            page.setController(controller);
            page.setUser(user);
            page.initialize(primaryStage);
    ...
    public static void main(String[] args) {
        ...
        // Add shutdown hook to ensure database is closed even if JavaFX fails to start
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("Emergency shutdown detected, closing database connection...");
            DatabaseManager.closeConnection();
        }));
